...
/obj/structure/deity/pylon/hear_talk(mob/speaker, list/phrases, verb = "says")
	...
	for(var/list/phrase in phrases)
		...
		to_chat(linked_god, "[html_icon(src)] <span class='game say'><span class='name'>[speaker]</span> (<A href='?src=\ref[linked_god];jump=\ref[src];'>P</A>) [verb], [linked_god.pylon == src ? "<b>" : ""]<span class='message'><span class='body'>\"[text]\"</span></span>[linked_god.pylon == src ? "</b>" : ""]</span>")
		if(linked_god.minions.len)
			for(var/minion in linked_god.minions)
				...
				if(mind.current && mind.current.eyeobj) //If it is currently having a vision of some sort
					to_chat(mind.current,"[html_icon(src)] <span class='game say'><span class='name'>[speaker]</span> (<A href='?src=\ref[src];vision_jump=\ref[src];'>J</A>) [verb], <span class='message'<span class='body'>\"[text]\"</span></span>")
