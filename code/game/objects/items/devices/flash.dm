	...
	if(iscarbon(M))
		if(M.stat!=DEAD)
			...
			if(safety < FLASH_PROTECTION_MODERATE)
				...
				if(flash_strength > 0)
					...
					ADJ_STATUS(M, STAT_BLURRY, flash_strength)
					ADJ_STATUS(M, STAT_CONFUSE, flash_strength + 2)
					if(flash_strength > 3)
						M.drop_held_items()
					if(flash_strength > 5)
		...
		if(safety < FLASH_PROTECTION_MAJOR)
			...
			if(safety < FLASH_PROTECTION_MODERATE)
				...
				SA.flash_eyes(2)
				ADJ_STATUS(SA, STAT_BLURRY, flash_strength)
				ADJ_STATUS(SA, STAT_CONFUSE, flash_strength)
		else 
			flashfail = 1
