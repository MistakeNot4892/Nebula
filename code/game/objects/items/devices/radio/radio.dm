var/datum/radio_channel/peer_to_peer_channel
...
var/list/initial_peer_to_peer_passwords = list()
...
/obj/item/radio/Initialize()
	...
	set_extension(src, /datum/extension/network_device, initial_network_id, initial_network_key, NETWORK_CONNECTION_WIRELESS)
...
/obj/item/radio/proc/sync_channels_with_network()
	...
	for(var/datum/extension/network_device/broadcaster in network?.devices)
		if(!istype(broadcaster.holder, /obj/machinery/network/telecomms_hub))
			continue
		var/obj/machinery/network/telecomms_hub/hub = broadcaster.holder
		if(!hub.can_receive_message() || !length(hub.channels))
			continue
...
/obj/item/radio/proc/can_receive_message()
	. = on
