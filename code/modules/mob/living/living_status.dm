...
/mob/living/proc/apply_pending_status_changes()
	for(var/condition in pending_status_counters)
		var/last_amount = LAZYACCESS(status_counters, condition)
		var/next_amount = LAZYACCESS(pending_status_counters, condition)
		if(last_amount != next_amount)
			world << "[src] applied: [condition], [last_amount || 0] => [next_amount || 0]"
