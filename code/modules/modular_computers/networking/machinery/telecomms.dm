...
/obj/machinery/network/telecomms_hub/Initialize()
	...
	if(isnull(channels))
		channels = GLOB.using_map.default_telecomms_channels.Copy()
	if(length(channels))
	...
	for(var/datum/extension/network_device/radio in network?.devices)
		if(istype(radio.holder, /obj/item/radio))
			var/obj/item/radio/R = radio.holder
			R.sync_channels_with_network()
...
/obj/machinery/network/telecomms_hub/proc/can_receive_message()
...
/obj/machinery/network/telecomms_hub/proc/get_recipients(var/list/levels, var/datum/computer_network/network, var/frequency)
	...
	for(var/datum/computer_network/use_network in receiving_networks)
		for(var/datum/extension/network_device/radio in use_network.devices)
			var/obj/item/radio/R = radio.holder
			if(!istype(R) || !R.can_receive_message())
				continue
	...
	if(!.)
		...
		if(href_list["factory_reset"])
			...
			if(confirm && CanPhysicallyInteract(user))
				channels_by_frequency = null
				channels_by_key = null
				channels = initial(channels) || GLOB.using_map.default_telecomms_channels.Copy()
				var/list/channel_data
				for(var/i = 1 to length(channels))
					var/datum/radio_channel/channel = new(channels[i])
					LAZYDISTINCTADD(channel_data, channel)
