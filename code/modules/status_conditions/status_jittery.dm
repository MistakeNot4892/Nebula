	...
	if(jitteriness >= 400)
		...
		if(prob(5))
			if(prob(1))
				...
				to_chat(src, SPAN_DANGER("Something bursts in your heart."))
				admin_victim_log(src, "has taken <b>lethal heart damage</b> at jitteriness level [jitteriness].")
			else
				...
				admin_victim_log(src, "has taken <i>minor heart damage</i> at jitteriness level [jitteriness].")
