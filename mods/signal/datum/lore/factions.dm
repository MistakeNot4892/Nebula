...
/decl/background_detail/faction/posthuman/get_random_cultural_name(mob/recipient, gender, species)
	var/decl/species/species_decl = istype(species, /decl/species) ? species : decls_repository.get_decl_by_id_or_var(species)
	switch(species_decl.uid)
		if(/decl/species/neoavian::uid)
			name_language = /decl/language/corvid
		//if(/decl/species/octopus::uid)
