/decl/language/varak
	...
	var/static/alist/replacement_word_map = list(
	...
	var/static/alist/precalculated_replacement_map = alist()

/decl/language/varak/Initialize()
	. = ..()
	if(length(replacement_word_map))
		for(var/word in replacement_word_map)
			for(var/replacement in replacement_word_map[word])
				precalculated_replacement_map[replacement] = word

/decl/language/varak/scramble_word(input, ends_sentence, input_ending)
	if(!length(precalculated_replacement_map))
		return ..()
	var/lookup = trim(lowertext(input))
	if(ends_sentence)
		lookup = copytext(lookup, 1, length(lookup))
	return precalculated_replacement_map[lookup] || ..()
